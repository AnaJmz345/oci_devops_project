...
function EditTaskModal({ open, onClose, onTaskUpdated, onTaskDeleted, task, sprints, users = [] }) {
  ...
  useEffect(() => {
    ...
    fetch(`/tasks/${task.taskId}/assignees`)
      ...
      .then((list) => {
        if (list.length > 0) {
          setAssignee(list[0]);
          setPendingHours(list[0].realTimeSpent ?? '');
          setForm((prev) => ({
            ...prev,
  ...
  const persistTask = async (realTimeSpent = null) => {
    setError('');
    setLoading(true);
    ...
    try {
      const body = {
        ...
        status: form.status === 'DONE' && realTimeSpent != null ? (task.status || 'TODO') : form.status,
      ...
      if (newOracleId && newOracleId !== currentOracleId) {
        ...
        await fetch('/tasks/assignees', {
          ...
          body: JSON.stringify({
            ...
            estimatedCompletionTime: Number.isNaN(est) ? null : est,
            realTimeSpent: realTimeSpent ?? 0,
          }),
        }).catch(() => {});
        ...
        if (!Number.isNaN(est)) {
          await fetch(`/tasks/assignees/${task.taskId}/${currentOracleId}/hours`, {
            ...
            body: JSON.stringify({
              ...
              realTimeSpent: realTimeSpent ?? undefined,
            }),
          }).catch(() => {});
        }
      }

      if (form.status === 'DONE' && newOracleId && realTimeSpent != null) {
        const completeRes = await fetch(`/tasks/${task.taskId}/complete`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            oracleId: newOracleId,
        ...
        if (!completeRes.ok) {
          ...
          throw new Error(msg || 'Could not save actual hours.');
        }
        updated = await completeRes.json();
      }
  ...
  const handleSave = async () => {
    ...
    if (form.status === 'DONE' && task?.status !== 'DONE') {
      const selectedAssigneeId = form.newAssigneeOracleId ? Number(form.newAssigneeOracleId) : null;
      if (!selectedAssigneeId) {
        setError('Assign a developer before marking this task as DONE.');
        return;
      }
      setCompletionError('');
  ...
  const selectedAssigneeName =
    developers.find((dev) => String(dev.oracleId) === String(form.newAssigneeOracleId || assignee?.oracleId))
      ?.name?.split(' ')?.[0] || null;
  ...
  return (
    <>
        ...
        <div className="TM-modal" data-testid="edit-task-modal">
          <div className="TM-header">
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              ...
              <button className="TM-close" onClick={onClose} aria-label="Close" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                x
              </button>
            </div>
          </div>
      ...
      <LogActualHoursModal
        ...
        taskName={form.taskName || task.taskName}
        assigneeName={selectedAssigneeName}
        initialHours={pendingHours}
        loading={loading}
        ...
        onConfirm={async (realTimeSpent) => {
          setCompletionError('');
          setPendingHours(realTimeSpent);
          try {
            await persistTask(realTimeSpent);
          } catch (err) {
